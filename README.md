...
PharmaHub is a pharmacy Inventory Management System with a built-in automata recognizer. The recognizer is the CCAUTOMA graded part: a minimized DFA that decides whether a pharmacy product code (like M001, S002, C001) is valid. Every medicine you add is checked by that DFA before it can be saved — that is how the two halves connect.
...
python cli/simulator.py --demo runs the 10 accepted + 10 rejected test cases. python cli/simulator.py --theory shows the NFA, subset construction and minimization step by step. (Or just double-click run-simulator.bat.)
...
Project layout
See docs/STRUCTURE.md for the folder tree and what each part does, and docs/SYSTEM_DESIGN.md for the architecture and how the DFA is used.
