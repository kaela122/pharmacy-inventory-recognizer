...
---
Quick reference (if you already know the drill)
Two terminals, both left running at the same time.
Backend terminal:
```
cd pharmacy-inventory-recognizer\backend
python -m uvicorn app.main:app --reload
```
Runs at http://127.0.0.1:8000 — API docs at `/docs`. First run auto-creates the database
and seeds demo data.
Frontend terminal:
```
...
npm run dev
```
Runs at http://localhost:5173 — open that in your browser. Vite proxies `/api` calls
through to the backend above.
New machine, or first time running it? Use `python -m pip install -r requirements.txt`
(backend) and `npm install` (frontend) once before the commands above — or just see the
...
```
   python -m uvicorn app.main:app --reload
   ```
Leave this window open. When it says `Application startup complete`, the backend is on.
